...
```
theaudiodb <command> [arguments] [flags]
```

Run `theaudiodb <command> --help` for the full flag list on any command. This
...
| `search <query>` | Search theaudiodb |
...
a row here per operation you declare in `theaudiodb/domain.go`.
