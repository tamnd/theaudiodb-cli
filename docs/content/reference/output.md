...
describes what users get. Pick a format with `-o`, or let theaudiodb choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
theaudiodb <command> -o table     # a rounded, color-aware grid for reading
theaudiodb <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
theaudiodb <command> -o list      # one record per section, easy on the eyes
theaudiodb <command> -o jsonl     # one JSON object per line, for piping
theaudiodb <command> -o json      # a single JSON array
theaudiodb <command> -o csv       # spreadsheet friendly
theaudiodb <command> -o tsv       # tab-separated
theaudiodb <command> -o url       # just the URL column
theaudiodb <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
theaudiodb <command> --fields id,title,url
```
...
```bash
theaudiodb <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
theaudiodb <command>            # a table, because this is a terminal
theaudiodb <command> | wc -l    # JSONL, because this is a pipe
```
