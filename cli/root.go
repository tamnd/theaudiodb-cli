// Package cli assembles the theaudiodb command tree from the theaudiodb
...
// NewApp assembles the kit application from the theaudiodb domain. The
...
//
// To add a command, declare it in theaudiodb/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := theaudiodb.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(theaudiodb.Domain{}).Register(app)
